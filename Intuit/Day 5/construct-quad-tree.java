...
class Solution {
    ...
    Node solve(int[][] grid, int x1, int x2, int y1, int y2) {
        if(x1 == x2) {
        ...
        int rowMid = (x1 + x2) / 2;
        int colMid = (y1 + y2) / 2;
        Node topleft = solve(grid, x1, rowMid, y1, colMid);                       
        Node topright = solve(grid, x1, rowMid, colMid + 1, y2);                       
        Node bottomleft = solve(grid, rowMid + 1, x2, y1, colMid);                       
        Node bottomright = solve(grid, rowMid + 1, x2, colMid + 1, y2);             
    ...
    public Node construct(int[][] grid) {
        return solve(grid,0,grid.length-1,0,grid[0].length-1);
    }
}